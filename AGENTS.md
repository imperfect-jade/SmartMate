...
本文件约束整个仓库。纯 `.h/.cpp` Qt Widgets 已完成迁移并作为 `SmartMate` 唯一前端。后续任何修改都必须保持以下 MVVM 边界。
...
- 正式且唯一的 `SmartMate` 前端必须使用 Qt Widgets。
- 新 View 必须位于 `src/view/widgets`，全部使用 `.h/.cpp` 构建，禁止 `.qml` 和 `.ui`。
- 禁止重新引入 Qt Quick、QuickTest、QML 注册 adapter、旧前端目标或迁移期条件构建。
- 正式程序和发布目录不得链接或携带 Qt QML/Quick 运行库与插件。
...
完整 `ctest` 仍是最终验收命令，上述筛选只用于 Widgets 开发时快速定位回归。
