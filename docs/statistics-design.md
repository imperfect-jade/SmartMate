...
> **实现状态**：首版任务统计纵向链路已经完整实现。Schema v4 任务事件、Model 聚合、Statistics Contracts/ViewModel、纯 Widgets + Qt Charts 仪表盘、主窗口导航、组合根注入、自动刷新和 Release 部署守卫均已接通。专注统计仍属于后续模块。
...
可执行与被阻塞描述任务的执行状态，即将到期与已经逾期描述截止时间风险，两组指标属于独立维度。同一任务可能既可执行或被阻塞，又属于即将到期或已经逾期，因此风险数量不得与执行状态数量直接相加。
...
当前 StatisticsPage 已在 View 层隔离使用 Qt Charts，但尚未加入正式主窗口；Charts 类型没有进入 Contracts、ViewModel、Model 或 Persistence。
...
- 部署检查要求存在 `Qt6Charts.dll`，同时拒绝 ChartsQml、Qt Graphs、QML/Quick 运行库和 `qml/` 目录。
