...
当前进度：六个阶段全部完成。删除前的 25 项双前端回归、QML lint 和旧入口冒烟均通过；获得明确批准后，旧 QML 前端、QuickTest、adapter、Qt Quick 依赖和迁移期构建开关已删除。最终 Debug/Release 仅构建正式 `SmartMate`，保留 22 项非 QML 测试。
...
阶段 2 当时额外把具体 ViewModel 核心目标与迁移期 QML 注册 adapter 分离，因此临时 `SmartMateWidgets` 的链接闭包不包含 Qt QML、Qt Quick、QML 插件或 `smartmate_ui`。该目标现已由正式 `SmartMate` 取代；Widgets 单元测试仍只注入 Fake Contract，纵向集成测试再组合具体 ViewModel 与 Service。
...
- Widgets 版本接管正式 `SmartMate`，并通过独立 Widgets-only 配置和发布目录验证。
- 删除前完成 25 项双前端回归、QML lint、旧入口冒烟并取得用户明确批准。
- 删除 `SmartMateQmlBaseline`、`smartmate_ui`、ViewModel QML adapter、全部 QML 文件和 QuickTest。
- 删除 Qt Quick/QML 组件、注册元数据、迁移开关、独立 preset 和 `all_qmllint`。
- 将 Widgets 入口规范为最终 `src/app/main.cpp`，同步更新架构守卫、部署脚本与文档。

完成标准：仓库不含 `.qml`、`.ui`、QML 注册元数据或 Qt Quick 链接；正式程序和发布目录只依赖 Qt Core/Gui/Widgets/Sql 及必要平台插件。该标准已完成。
...
迁移完成后，完整的 Model、Persistence、ViewModel、Widget、集成、应用冒烟和架构测试共同构成最终回归基线。后续开发不得通过放宽 Model 规则、复制算法或跳过原子端口破坏既有边界。
