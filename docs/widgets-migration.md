...
当前进度：阶段 1、2、3、4、5 已完成，阶段 6 已到达删除确认门禁。Widgets 已接管正式 `SmartMate`；迁移配置 25 项测试、Widgets-only 配置 22 项测试、QML lint 和纯 Widgets Release 发布均已通过。旧前端以 `SmartMateQmlBaseline` 临时保留，等待用户批准删除清单。
...
- Widgets 版本接管正式 `SmartMate`，旧前端临时改名为 `SmartMateQmlBaseline`。
- 使用 `SMARTMATE_BUILD_QML_BASELINE=OFF` 和独立 preset 证明正式目标不需要 QML 构建链。
- 完整回归与纯 Widgets 发布验证通过后，向用户报告删除清单并暂停。
- 已取消临时 `SmartMateWidgets` 名称；用户批准后再删除 `smartmate_ui`、QML 插件、QML 文件和 QuickTest。
- 删除 Qt Quick、QML、QuickControls2、QuickTest 依赖及所有 QML 注册元数据。
- 更新 README、项目概览、部署文档和 `AGENTS.md`，移除迁移期例外与 `all_qmllint`。

删除操作只有在用户明确批准后才能执行。完成标准：仓库不含 `.qml`、`.ui`、QML 注册元数据或 Qt Quick 链接；正式程序和发布目录只依赖 Qt Core/Gui/Widgets/Sql 及必要平台插件。
