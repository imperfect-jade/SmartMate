...
SmartMate 的正式 View 已切换为纯 `.h/.cpp` Qt Widgets。阶段 6 的删除门禁前仍保留可运行的 Qt Quick/QML 行为基线，用于完成最后一次等价验证；该基线不再是正式入口。切换只替换展示技术并收窄 View 与 ViewModel 的编译期依赖，不改变 Model 业务规则、Service API、Repository 事务语义、数据库格式或用户数据兼容性。
...
正式 `SmartMate` 目标使用 Qt Widgets。旧 Qt Quick/QML 程序临时保留为 `SmartMateQmlBaseline`，仅用于删除前的行为、样式和构建对照：

- 现有 QML 必须继续通过构建、QuickTest、离屏启动和 `all_qmllint`，但不得新增页面、ViewModel API 或 Qt Quick 功能。
- 迁移期 QML 类型注册集中在独立 `smartmate_viewmodel_qml` adapter；具体 `smartmate_viewmodel` 保持 Qt Core/Model/Contracts 目标，使 Widgets 进程不携带 QML 运行库。
- `SMARTMATE_BUILD_QML_BASELINE=OFF` 时不得查找、构建或链接 QML、Qt Quick、QuickTest、adapter 或旧前端测试。
- 两个前端必须消费相同的 Model Service，不得复制状态机、依赖、排序、统计或持久化规则。
- 自动测试只使用内存数据库；不得让两个前端同时操作真实用户数据库。
- 必须先完成迁移配置、Widgets-only 配置和纯 Widgets 发布目录验证，再列出删除清单并暂停。
- 只有用户明确批准后，才可删除 `SmartMateQmlBaseline`、QML、QuickTest、Qt Quick 依赖和 QML 注册元数据。

完整阶段、退出条件与最终清理项见[Qt Widgets 迁移指南](widgets-migration.md)。
...
任务主流程按展示职责拆分为三个彼此独立的投影：`TaskListViewModel` 只负责列表、筛选、状态命令和批量选择，`TaskFocusViewModel` 负责不受列表筛选影响的“现在做”投影，`TaskDetailsViewModel` 负责稳定 `TaskId` 驱动的详情会话。三者不互相调用，分别监听同一个 `TaskService` 的变化信号；删除门禁前的 QML 基线也必须使用这三个独立端口，禁止把焦点或详情状态重新塞回列表 Contract。

### 3.5 Qt Widgets View

最终 View 全部位于 `src/view/widgets`，只使用 `.h/.cpp` 构建，不使用 `.ui`。View 负责控件创建、布局、绘制、样式、动画、焦点、对话框显示、缩放平移和用户事件转发。
...
SmartMateQmlBaseline           → app core + QML adapter + smartmate_ui（删除门禁前）
```

`smartmate_widgets` 永远不能链接具体 `smartmate_viewmodel`；`smartmate_viewmodel` 永远不能链接 `smartmate_widgets` 或 `smartmate_persistence`。只有组合根可同时链接具体实现和 View。删除门禁前的 `smartmate_viewmodel_qml`、`smartmate_ui` 和 QML 插件只能被 `SmartMateQmlBaseline` 与 QuickTest 使用，不得进入正式 `SmartMate` 的链接闭包。
