...
`TaskEditorViewModel`在新建模式下同时维护任务字段和前置任务选择草稿。两者以一个`TaskCreationRequest`提交给Service，不能由两个ViewModel分步保存。编辑草稿不包含可写状态：新建时只展示固定的待办状态，编辑时只展示当前状态；所有转换必须经由任务列表命令。编辑已有任务时，它只能通过`TaskService::findEditableTask`建立草稿；归档任务由Model拒绝，ViewModel仅映射错误。类型化选择器可以检查日期和分段输入格式，但总分钟数等领域边界必须委托Model校验。
