...
统计阶段的状态转换必须在同一 Repository 事务中同时更新任务快照并插入对应任务事件。单项命令使用单元素原子转换批次，批量归档与恢复则对全部状态和事件整批提交或整批回滚；Repository 查询端口只返回普通事件，不在 SQL 中生成业务汇总。Schema v3 升级到计划中的 Schema v4 时不得根据 `updatedAtUtc` 伪造旧完成历史，永久删除任务必须级联删除其事件。

`SqliteTaskRepository` 继续是实现多个窄任务 Repository 端口的单一 SQLite Adapter，并由一个实例独占同一连接。其成员实现按连接/Schema、任务 CRUD、依赖边、跨表原子命令和类别端口拆入多个 `.cpp`，但不会创建第二个 Adapter 或隐藏连接。`persistence/internal` 只保存稳定列协议、查询行映射、参数绑定、Schema 检查和 SQL 错误收敛；它不是 Model 公共 API。原子创建、批量状态、永久删除及类别删除的 `begin/commit/rollback` 必须分别完整保留在单一 Repository 方法中，不得因文件拆分破坏事务边界。
...
2. `tests/architecture/check_mvvm_boundaries.cmake` 检查禁用 include、链接、Controller、EventBus、Service Locator、SQL 和 View API 泄漏；统计阶段还要拒绝 Charts 类型进入非 View 层，并继续拒绝 QML/Quick。
