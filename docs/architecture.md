...
数据统计的 Model 数据底座、Statistics Contracts 与具体 ViewModel 已经实现，Widgets 仪表盘、主窗口导航和组合根注入尚未实现，详细口径与阶段见[数据统计设计](statistics-design.md)。后续阶段允许把 Qt Charts 作为 `smartmate_widgets` 的私有 View 依赖；该例外不允许引入 Qt Graphs、`QQuickWidget`、QML 或 Qt Quick，也不允许 Charts 类型越过 View 边界。
...
统计阶段由同一组合根创建任务事件 Repository 实现、`StatisticsService` 和 `StatisticsViewModel`，并只把 Statistics Contract 引用加入 `MainWindowDependencies`。统计 ViewModel 不调用其他子 ViewModel；状态写入后的 `TaskService` 失效信号触发其独立重投影。
...
统计功能实现完成后的目标关系仅增加一条 View 私有依赖：

```text
smartmate_widgets              → contracts + smartmate_common
...
### 5.4 数据统计（Model 与 ViewModel 已实现、Widgets 待实现）
  ...
  view/
    widgets/
      ...
      graph/
        ...
        DependencyGraphView.*
      statistics/              # 计划中的纯 Widgets + Qt Charts 页面
      settings/
```
