...
数据统计的 Model 数据底座已经实现，Statistics Contracts、ViewModel 与 Widgets 仪表盘尚未实现，详细口径与阶段见[数据统计设计](statistics-design.md)。后续阶段允许把 Qt Charts 作为 `smartmate_widgets` 的私有 View 依赖；该例外不允许引入 Qt Graphs、`QQuickWidget`、QML 或 Qt Quick，也不允许 Charts 类型越过 View 边界。
...
src/
  ...
  viewmodel/
    ...
    StatisticsViewModel.*      # 计划中的统计展示投影
  view/
    widgets/
      MainWindow.*
      binding/
