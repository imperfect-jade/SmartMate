...
大型具体 ViewModel 可以把同一个类的成员实现按职责放入多个 `.cpp`，但不能借文件拆分引入第二套状态、隐藏协作者或新的公开端口。`TaskEditorViewModel` 的展示核心、会话命令与草稿状态仍共享同一 Contract 和生命周期；`TaskListViewModel` 的列表核心、单项命令、批量选择及可见投影仍由同一个实例维护。跨文件调用只发生在该类的私有成员之间，Model 资格来源、共享投影源和 Widget 绑定边界不变。
