...
SmartMate 正在把当前 Qt Quick/QML View 迁移为纯 `.h/.cpp` Qt Widgets View。迁移只替换展示技术并收窄 View 与 ViewModel 的编译期依赖，不改变 Model 业务规则、Service API、Repository 事务语义、数据库格式或用户数据兼容性。
...
迁移采用双前端策略：当前 `SmartMate` Qt Quick/QML 程序仅作为行为基线，新的纯 C++ Widgets 前端使用临时 `SmartMateWidgets` 目标。迁移期间：

- 现有 QML 必须继续通过构建、QuickTest、离屏启动和 `all_qmllint`，但不再新增页面、ViewModel API 或 Qt Quick 功能。
...
- 新功能开发暂停；只有行为保持、回归修复和阻断 Widgets 迁移的问题可以修改当前 QML。
- 两个前端必须消费相同的 Model Service，不得复制状态机、依赖、排序、统计或持久化规则。
- 每个纵向切片完成行为等价验证后才算迁移完成；在全部切片完成前不删除当前 QML 基线。
- 最终由 Widgets 版本接管正式 `SmartMate` 目标，再删除临时目标、QML、QuickTest、Qt Quick 依赖和 QML 注册元数据。
...
任务主流程按展示职责拆分为三个彼此独立的投影：`TaskListViewModel` 只负责列表、筛选、状态命令和批量选择，`TaskFocusViewModel` 负责不受列表筛选影响的“现在做”投影，`TaskDetailsViewModel` 负责稳定 `TaskId` 驱动的详情会话。三者不互相调用，分别监听同一个 `TaskService` 的变化信号；QML 迁移期也必须使用这三个独立端口，禁止把焦点或详情状态重新塞回列表 Contract。
...
smartmate_viewmodel_qml        → smartmate_viewmodel + Qt6::QmlIntegration（迁移期）
smartmate_widgets              → contracts + smartmate_common + Qt6::Widgets
SmartMateWidgets               → persistence + concrete viewmodel + widgets
```

迁移期临时保留当前 `smartmate_ui`、QML 插件和正式 `SmartMate`，并增加 `SmartMateWidgets`。`smartmate_widgets` 永远不能链接具体 `smartmate_viewmodel`；`smartmate_viewmodel` 永远不能链接 `smartmate_widgets` 或 `smartmate_persistence`。只有组合根可同时链接具体实现和 View。
