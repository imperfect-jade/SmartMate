...
`TaskEditorViewModel`在新建模式下同时维护任务字段和前置任务选择草稿。两者以一个`TaskCreationRequest`提交给Service，不能由两个ViewModel分步保存。编辑草稿不包含可写状态：新建时只展示固定的待办状态，编辑时只展示当前状态；所有转换必须经由任务列表命令。编辑已有任务时，它只能通过`TaskService::findEditableTask`建立草稿；归档任务由Model拒绝，ViewModel仅映射错误。`TaskGraphViewModel`只把Model提供的结构化图快照转换为节点像素坐标、贝塞尔控制点和箭头几何；拓扑层级与可见节点闭包仍由Model计算。
...
依赖图读取链路为：`SQLite Repository → TaskService结构化图快照 → TaskGraphViewModel布局与箭头几何 → QML Shape渲染`。Model负责最长前置链层级、活动任务及其相连归档节点闭包、边解析状态和异常图拒绝；ViewModel负责像素尺寸、坐标以及完成/待满足/取消三态投影；View只使用绿色、橙色、灰色绘制，平移、缩放并转发稳定任务ID。布局、缩放和选中状态不持久化。
