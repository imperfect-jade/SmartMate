...
> 当前版本已经完成基础任务 CRUD、Finish-to-Start依赖、原子新建依赖、有向依赖图、依赖感知排序、推荐理由、实时搜索和优先级筛选。主窗口提供可折叠左侧导航、“现在做”任务槽、卡片详情以及可持久化的青绿/清蓝与字体设置。任务与依赖通过 SQLite 保留，外观偏好通过 persistence 层的 QSettings 保留；桌宠、专注计时和统计仍按后续阶段逐步实现。
...
- 任务管理（已实现）：新建、查看和编辑任务字段；任务固定从待办开始，并通过“开始、取消、完成、重做、归档、恢复”按钮按Model状态机转换。归档任务必须恢复后才能编辑，新建时可同时选择前置任务并与任务原子保存。
...
- 有向依赖图（已实现）：从左向右显示全部活动任务及相关归档节点，以绿色、橙色和灰色箭头区分完成、待满足和取消关系；支持平移、缩放、适应窗口和节点详情。
- 智能规划（已实现）：可执行任务优先，被阻塞任务按拓扑关系整理；前置任务完成或取消后自动解锁后继并显示可解锁数量。
