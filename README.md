...
这样可以让任务列表、依赖图、桌宠和统计页面以不同方式展示同一份业务状态，同时保持算法和数据库代码可独立测试。依赖图继续遵循 Model 计算拓扑语义、ViewModel 计算像素布局和箭头几何、View 只绘制并转发稳定 ID。详细约束见[架构说明](docs/architecture.md)与[仓库开发规则](AGENTS.md)。
