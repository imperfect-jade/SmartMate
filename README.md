...
> 当前版本已经完成基础任务 CRUD、Finish-to-Start依赖、原子新建依赖、有向依赖图、依赖感知排序、推荐理由、实时搜索和优先级筛选。任务与依赖通过 SQLite 在重启后保留；桌宠、专注计时和统计仍按后续阶段逐步实现。

## 主要功能规划

- 任务管理（已实现）：新建、查看、编辑、归档、恢复任务，并维护状态、优先级、截止时间和预计用时；新建时可同时选择一个或多个前置任务，任务与关系在同一SQLite事务中保存。
- 查询与整理（已实现）：实时搜索标题或描述，按单个优先级筛选；Model 自动生成稳定顺序和推荐理由，ViewModel 只生成当前列表投影。搜索和筛选条件不会持久化。
- 任务依赖（已实现）：新建或编辑待办任务时选择一个或多个前置任务，拒绝非法端点、重复依赖和循环依赖，并显示直接阻塞原因。
- 有向依赖图（已实现）：从左向右显示全部活动任务及相关归档节点，以绿色或橙色箭头区分已满足和未满足关系；支持平移、缩放、适应窗口和节点详情。
- 智能规划（已实现）：可执行任务优先，被阻塞任务按拓扑关系整理；完成前置任务后自动解锁后继并显示可解锁数量。
